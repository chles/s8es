...
(function(f) {
    ...
    return function e(t, n, r) {
        ...
        8: [function(require, module, exports) {
            module.exports = {
                client_id: "e5f4f09d2b341a90da3eee85d92aa2b9"
            }
        }, {}],
        9: [function(require, module, exports) {
            ...
            plangular.directive("plangular", ["$rootScope", "$timeout", "$http", "$q", "Auth", "$modal", "plangularConfig", function($rootScope, $timeout, $http, $q, Auth, $modal, plangularConfig) {
                ...
                return {
                    ...
                    link: function(scope, elem, attr) {
                        ...
                        // Allow stop if player is playing when url changed
                        player.pause();
                        ...
                        function getFullData(likes) {
                            var deferred = $q.defer();
                            var req = [];

                            angular.forEach(likes, function (value, key) {
                                req[key] = $http.get('https://api.soundcloud.com/tracks/'+value+'.json?client_id=e5f4f09d2b341a90da3eee85d92aa2b9');
                            });

                            $q.all(req).then(function(result) {
                                var list = [];
                        ...
                        if (src) {
                            ...
                            if (src.indexOf('http') === -1 ) {
                                ...
                                var ref = new Firebase("https://s8essound.firebaseio.com//favorites/"+src+"/likes");
                                
                                ref.on("value", function(snapshot) {

                                    var likes = [];

                                    for (var ids in snapshot.val()) {
                                        likes.push(snapshot.val()[ids]);
                                        console.log(likes);
                                      //console.log("o." + prop + " = " + obj[prop]);
                                    }

                                    getFullData(likes).then(function(playlist) {
                                        //scope.$apply(function() {
                                        $timeout(function() {
                                            scope.$apply(function() {
                                                //console.log(playlist);
                                                scope.track = createSrc(playlist);
                                                if (Array.isArray(playlist)) {
                                                    //scope.tracks = scope.tracks.concat(playlist);
                                                    scope.tracks = playlist.map(function(track) {
                                                        //console.log('YES');
                                                    ...
                                                    scope.tracks = playlist.tracks.map(function(track) {
                                                        return createSrc(track)
                                                    })
                                                }
                                                //scope.tracks = scope.tracks.concat(playlist);
                        ...
                        scope.like = function (id) {
                            var url = firebaseUrl+'favorites/'+$rootScope.displayName+'/likes';
                            if(Auth.$getAuth()){
                                
                                var ind = $rootScope.liked.indexOf(id);

                                /*if (src.indexOf('http') === -1 ) {
                                    console.log($rootScope.liked);
                                    scope.tracks.splice(ind, 1);
                                    console.log(scope.tracks);
                                ...
                                // This is a dislike action
                                if(ind > -1){    
                                    var dislike = new Firebase(url+'/'+$rootScope.ids[ind]);
                                    ...
                                    $rootScope.liked.splice(ind, 1);
                                    $rootScope.ids.splice(ind, 1);

                                    dislike.remove();
                                }else{
                                    var thislike = new Firebase(url);
                                    thislike.push(id);

                                    $rootScope.liked.push(id);
